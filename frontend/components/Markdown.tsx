        ...
        components={{
          a: ({ href, children }) => (
            <a
              ...
              className="md-link underline decoration-slate-300 hover:decoration-blue-400 text-blue-600 dark:text-blue-400"
            >
              {children}
            </a>
          ),
          code: (p) => {
            ...
            return (
              <pre className="md-pre overflow-x-auto rounded-xl p-4 my-4 text-[0.9em] bg-slate-50 dark:bg-neutral-900 border border-slate-200 dark:border-neutral-800 shadow-sm">
                <code className={className}>{children}</code>
              </pre>
            );
          },
          p: ({ children }) => <p className="md-p whitespace-pre-wrap leading-relaxed mb-4">{children}</p>,
          ...
          blockquote: ({ children }) => (
            <blockquote className="border-l-4 border-blue-400 dark:border-blue-600 pl-4 py-2 my-4 italic text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-neutral-900/50 rounded-r-lg">
              {children}
            </blockquote>
          ),
          table: ({ children }) => (
