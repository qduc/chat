...
case "$cmd" in
    up)
        # Note: ENCRYPTION_MASTER_KEY will be auto-generated by the entrypoint if not provided
        # To use a custom key, set the environment variable before running prod.sh:
